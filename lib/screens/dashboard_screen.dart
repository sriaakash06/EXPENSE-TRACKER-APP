...
class DashboardScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // To compute previous month:
    List<int> parts = provider.globalSelectedMonthYear.split('-').map(int.parse).toList();
    DateTime curMonth = DateTime(parts[1], parts[0], 1);
    DateTime prevMonth = DateTime(parts[1], parts[0] - 1, 1);
    final lastMonthSpent = provider.getExpensesByMonth(prevMonth.month, prevMonth.year).fold(0.0, (sum, e) => sum + e.amount);
    ...
    final diffText = lastMonthSpent > totalSpent 
        ? '$percentDiff% below previous month' 
        : (lastMonthSpent == 0 ? 'No prior month data' : '$percentDiff% above previous month');
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // This Month Spend
                  Text(
                    'Spend in ${DateFormat('MMMM yyyy').format(curMonth)}',
                    style: TextStyle(
